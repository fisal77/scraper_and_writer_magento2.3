# this uses output from previous script, reads data files and prepares html etc. and then produces csv file to update
### for running from cron setup full path on server below
...
for item_f in item_folders:
    try:
        ...
        #define category
        categories = "Default Category/" + json_data.get('group').strip().replace(" - ","$-$") .replace(" -","-") .replace("- ","-") .replace("$-$"," - ") + '/' + json_data.get('category').strip() + '/' + json_data.get('subcategory').strip() .replace(" - ","$-$") .replace(" -","-") .replace("- ","-") .replace("$-$"," - ")
        ...
        data_to_write.update({'images': ",".join(images)})
